...
class AdvancedExample2TileMap
{
    ...
    private function initExample():Void {
        ...
        var texture:GTexture = GTextureManager.createTextureFromBitmapData("rect", new BitmapData(58,58,false,0xFF0000));
        ...
        tileMap.node.onMouseUp.add(mouseUpHandler);
    ...
    private function mouseUpHandler(input:GMouseInput):Void {
		trace("here");
    }
    ...
    private function mouseHandler(signal:GMouseInput):Void {
        if (!signal.buttonDown) {
            ...
            if (omx != -1) {
                var tx:Float = camera.node.x - (signal.localX-omx);
                var ty:Float = camera.node.y - (signal.localY-omy);
